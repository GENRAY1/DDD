...
public class AddUserByTopicCommandHandler(IUserProducers producer) : IRequestHandler<AddUserByTopicCommand>
{
    /// <summary>
    ...
    public Task Handle(AddUserByTopicCommand request, CancellationToken cancellationToken)
    {
        //отправляем сообщение по шине
        producer.UserCreateByTopicProducer(new UserDto(request.FirstName, request.LastName, request.PhoneNumber, request.Email,
            request.Patronymic));
