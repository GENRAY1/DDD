...
public class AddUserCommandHandler(IUserProducer producer) : IRequestHandler<AddUserCommand>
{
    /// <summary>
    ...
    public Task Handle(AddUserCommand request, CancellationToken cancellationToken)
    {
        ...
        producer.ProduceUserCreate(new UserDto(request.FirstName, request.LastName, request.PhoneNumber, request.Email,
            request.Patronymic));
